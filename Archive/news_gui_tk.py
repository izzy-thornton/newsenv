...
class NewsScraperApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Radiobutton(row1, text="Start new scrape", variable=self.var_mode, value="new").pack(side="left", padx=(0, 18))
        ttk.Radiobutton(row1, text="Append to existing and deduplicate", variable=self.var_mode, value="append").pack(side="left")

        row2 = ttk.Frame(frm_params)
        row2.pack(fill="x", pady=2)
        ttk.Label(row2, text="Select topic:", width=20, font=("Montserrat", 12)).pack(side="left")
        self.cbo_topic = ttk.Combobox(
            row2,
            values=list(SEARCH_PRESETS.keys()),
            textvariable=self.var_topic,
            state="readonly",
            width=42,
        )
        self.cbo_topic.pack(side="left")
