...
class NewsScraperApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self._wordcloud_pil: Image.Image | None = None
        self._wordcloud_fullscreen_photo: ImageTk.PhotoImage | None = None
        self._wordcloud_zoom = 1.0
    ...
    def _build_ui(self) -> None:
        ...
        def _on_canvas_configure(event):
            ...
            self._refresh_wordcloud_preview()
        ...
        self.lbl_wc = ttk.Label(frm_out, cursor="hand2")
        self.lbl_wc.pack(fill="both", expand=True)
        self.lbl_wc.bind("<Button-1>", lambda event: self._open_wordcloud_viewer())
    ...
    def _set_wordcloud_image(self, png_path: Path | None) -> None:
        if not png_path or not png_path.exists():
            self.lbl_wc.configure(image="", text="")
            self._wordcloud_photo = None
            self._wordcloud_pil = None
            return

        try:
            self._wordcloud_pil = Image.open(png_path)
            self._refresh_wordcloud_preview()
        except Exception:
            self.lbl_wc.configure(image="", text="")
            self._wordcloud_photo = None
    ...
    def _refresh_wordcloud_preview(self) -> None:
        if self._wordcloud_pil is None:
            self.lbl_wc.configure(image="", text="")
            self._wordcloud_photo = None
            return

        try:
            available_width = max(400, self.left_canvas.winfo_width() - 60) if hasattr(self, "left_canvas") else 800
            img = self._wordcloud_pil.copy()
            width, height = img.size
            scale = min(1.0, available_width / max(1, width))
            preview_w = max(300, int(width * scale))
            preview_h = max(150, int(height * scale))
            img = img.resize((preview_w, preview_h))
            self._wordcloud_photo = ImageTk.PhotoImage(img)
            self.lbl_wc.configure(image=self._wordcloud_photo, text="")
        except Exception:
            self.lbl_wc.configure(image="", text="")
            self._wordcloud_photo = None

    def _open_wordcloud_viewer(self) -> None:
        if self._wordcloud_pil is None:
            return

        top = tk.Toplevel(self)
        top.title("Word Cloud Viewer")
        ...
        zoom_var = tk.StringVar(value=f"{int(self._wordcloud_zoom * 100)}%")

        viewer_canvas = tk.Canvas(container, highlightthickness=0)
        xscroll = ttk.Scrollbar(container, orient="horizontal", command=viewer_canvas.xview)
        yscroll = ttk.Scrollbar(container, orient="vertical", command=viewer_canvas.yview)
        viewer_canvas.configure(xscrollcommand=xscroll.set, yscrollcommand=yscroll.set)

        yscroll.pack(side="right", fill="y")
        xscroll.pack(side="bottom", fill="x")
        viewer_canvas.pack(side="left", fill="both", expand=True)

        def render_view():
            img = self._wordcloud_pil.copy()
            w, h = img.size
            new_w = max(200, int(w * self._wordcloud_zoom))
            new_h = max(100, int(h * self._wordcloud_zoom))
            resized = img.resize((new_w, new_h))
            self._wordcloud_fullscreen_photo = ImageTk.PhotoImage(resized)
            viewer_canvas.delete("all")
            viewer_canvas.create_image(0, 0, anchor="nw", image=self._wordcloud_fullscreen_photo)
            viewer_canvas.configure(scrollregion=(0, 0, new_w, new_h))
            zoom_var.set(f"{int(self._wordcloud_zoom * 100)}%")

        def zoom_in():
            self._wordcloud_zoom = min(5.0, self._wordcloud_zoom * 1.2)
            render_view()

        def zoom_out():
            self._wordcloud_zoom = max(0.2, self._wordcloud_zoom / 1.2)
            render_view()
        ...
        ttk.Button(toolbar, text="-", width=3, command=zoom_out).pack(side="left", padx=(8, 4))
        ttk.Button(toolbar, text="+", width=3, command=zoom_in).pack(side="left", padx=4)
        ttk.Button(toolbar, text="100%", command=zoom_reset).pack(side="left", padx=4)
        ttk.Label(toolbar, textvariable=zoom_var).pack(side="left", padx=(8, 0))
        ttk.Label(toolbar, text="Click and use scrollbars to inspect the full image.").pack(side="right")
        ...
        top.bind("<Control-MouseWheel>", on_ctrl_mousewheel)
        render_view()
